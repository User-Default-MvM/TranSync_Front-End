...
const ForgotPassword = () => {
  ...
  // Referencia para el campo de email
  ...
  const handleKeyDown = (e, isSubmit = false) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (isSubmit) {
        handleSubmit(e);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validaciones
    if (!email) {
      setError(t('forgotPassword.messages.emailRequired'));
      return;
    }

    if (!isEmailValid(email)) {
      setError(t('forgotPassword.messages.emailInvalid'));
      return;
    }
    ...
    try {
      ...
      setSuccess(t('forgotPassword.messages.success'));
    } catch (err) {
      console.error("Error en forgot password:", err);
      setError(err.message || t('forgotPassword.messages.error'));
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="lg:w-2/5 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 p-4 sm:p-6 md:p-8 lg:p-12 xl:p-16 flex flex-col justify-center relative overflow-hidden animate-slide-in-left">
            ...
            <div className="relative z-10">
              ...
              <p className="text-blue-100 text-sm sm:text-base md:text-lg xl:text-xl mb-6 sm:mb-8 md:mb-10 leading-relaxed min-w-0">
                <span className="truncate">{t('forgotPassword.brand.description')}</span>
              </p>
              <div className="space-y-3 sm:space-y-4 md:space-y-6">
                <div className="flex items-center text-blue-100 text-sm sm:text-base md:text-lg min-w-0">
                  ...
                  <span className="truncate">{t('forgotPassword.brand.features.security')}</span>
                </div>
                <div className="flex items-center text-blue-100 text-sm sm:text-base md:text-lg min-w-0">
                  <FaUsers className="mr-2 sm:mr-3 md:mr-4 text-blue-300 text-base sm:text-lg md:text-xl flex-shrink-0" />
                  <span className="truncate">{t('forgotPassword.brand.features.admin')}</span>
                </div>
                <div className="flex items-center text-blue-100 text-sm sm:text-base md:text-lg min-w-0">
                  <FaCogs className="mr-2 sm:mr-3 md:mr-4 text-blue-300 text-base sm:text-lg md:text-xl flex-shrink-0" />
                  <span className="truncate">{t('forgotPassword.brand.features.dashboard')}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4 sm:space-y-6 md:space-y-8">
              ...
              <div>
                <label htmlFor="email" className="block text-xs sm:text-sm font-semibold text-text-primary-light dark:text-slate-200 mb-2 sm:mb-3 min-w-0">
                  <span className="truncate">{t('forgotPassword.form.email')} <span className="text-red-500">*</span></span>
                </label>
              ...
              <button
                ...
                disabled={loading}
                aria-describedby={loading ? "forgot-loading" : undefined}
              >
                {loading ? (
                  <div className="flex items-center justify-center">
                    <FaSpinner className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mr-2 sm:mr-3 animate-spin flex-shrink-0" />
                    <span className="truncate">{t('forgotPassword.form.sending')}</span>
                  </div>
                ) : (
                  <span className="truncate">{t('forgotPassword.form.submitButton')}</span>
                )}
              </button>
              ...
              <div className="text-center pt-3 sm:pt-4 md:pt-6 border-t border-border-light dark:border-gray-700">
                <p className="text-text-secondary-light dark:text-slate-300 text-xs sm:text-sm md:text-base mb-2 sm:mb-3 md:mb-4 min-w-0">
                  <span className="truncate">{t('forgotPassword.form.rememberPassword')}</span>
                </p>
                <button
                  type="button"
                ...
                >
                  <span className="truncate">{t('forgotPassword.form.backToLogin')}</span>
                </button>
              </div>
            </div>
          </div>
